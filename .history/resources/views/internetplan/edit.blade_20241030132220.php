                        ...
                        <div class="card-body">
                            <form action="{{route('internetplan.update', $internetplan->id)}}" method="POST">
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col">
                                        <div class="col-sm-12">
                                                {{--  <select name="bandwidth_name" class="custom-select">
                                                    <option value="{{$internetplan->bandwidth_name}}">

                                                        @if (!@empty($internetplan->bandwidth))
                                                            {{$internetplan->bandwidth_name}}
                                            ...
                                            <select name="bandwidth_name" class="custom-select">
                                                @if (!empty($internetplan->bandwidth_name))
                                                    <!-- Display the selected bandwidth name as the default selected option -->
                                                    <option value="{{ $internetplan->bandwidth_name }}" selected>
                                                        {{ $internetplan->bandwidth_name }}
                                                    </option>
                                                @endif

                                                <!-- Loop through the bandwidths array and display each as an option -->
                                                @foreach($bandwidths as $bandwidth)
                                                    <option value="{{ $bandwidth }}"
                                                        {{ isset($internetplan->bandwidth_name) && $internetplan->bandwidth_name == $bandwidth ? 'selected' : '' }}>
                                                        {{ $bandwidth }}
                                                    </option>
                                                @endforeach
                                            </select>
                                    ...
                                    <div class="col">
                                        <div class="col-sm-4">
                                            <select name="type" class="custom-select">
                                                <option value="{{$internetplan->type}}">{{$internetplan->type}}</option>
                                            </select>
                                        </div>
                                    </div>
                                </div>
