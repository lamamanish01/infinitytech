                                ...
                                <tbody>
                                    @forelse($internetplans as $internetplan)
                                        <tr>
                                            ...
                                            <td>{{$internetplan->upload_rate}}</td>
                                            <td>{{$internetplan->download_rate}}</td>
                                            <td>
                                                <div class="btn-group">
                                                    @can('edit roles')
                                                        <a href="{{route('bandwidth.edit', $internetplan->id)}}" class="btn btn-sm btn-secondary">Edit</a>
                                                    @endcan
                                                    @can('delete roles')
                                                        <form action="{{route('bandwidth.destroy', $internetplan->id)}}" method="post">
                                                            @method('Delete')
                                                            @csrf
