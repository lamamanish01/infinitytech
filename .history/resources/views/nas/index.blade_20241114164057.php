                                ...
                                <tbody>
                                    @forelse($nases as $nas)
                                        <tr>
                                            ...
                                            <td>{{$nas->name}}</td>
                                            <td>{{$nas->bandwidth_name}}</td>
                                            <td>{{$nas->price}}</td>
                                            <td>{{$nas->duration}}</td>
                                            <td>
                                                <div class="btn-group">
                                                    @can('edit nas')
                                                        <a href="{{route('internetplan.edit', $nas->id)}}" class="btn btn-sm btn-secondary">Edit</a>
                                                    @endcan
                                                    @can('delete nas')
                                                        <form action="{{route('internetplan.destroy', $nas->id)}}" method="post">
                                                            @method('Delete')
                                                            @csrf
