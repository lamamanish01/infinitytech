...
class UpdateExpiredCustomers extends Command
{
    ...
    {
        $recharges = Recharge::whereNull('expire_date')->get();
        if ($recharges->isEmpty()) {
            $this->info("No customers with a null 'expire_date' were found.");
        } else {
            foreach ($recharges as $recharge) {
                $this->info("Customer {$recharge->id} found with null 'expire_date'.");
            }
        }
        ...
        foreach ($expiredCustomers as $expiredCustomer)
        {
            ...
            $this->info("Customer {$expiredCustomer->username}'s Framed-Pool updated to expired-pool.");
        }
    }
