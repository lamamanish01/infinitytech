...
class UpdateExpiredCustomers extends Command
{
    ...
    public function handle()
    {
        ...
        $expiredCustomers = DB::table('radcheck')
        ->where('attribute', 'Expiration')
        ->where('value', '<', Carbon::now()->toDateTimeString())
        ->get();
        $expiredCustomers = RadCheck::where('attribute', '=', $expiredCustomers)
        where('Expiration', '<', now())->get();
