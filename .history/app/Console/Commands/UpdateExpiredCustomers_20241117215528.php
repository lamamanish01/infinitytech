...
class UpdateExpiredCustomers extends Command
{
    ...
    private function disconnectCustomer($username)
    {
        ...
        nasIp = env('RADIUS_NAS_IP', $session->nasipaddress); // Use from `radacct` or default from `.env`
        $port = env('RADIUS_PORT', 1700);                      // Default to 1700
        $sharedSecret = env('RADIUS_SHARED_SECRET', 'testing123');
