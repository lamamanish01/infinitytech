<?php

namespace App\Http\Controllers;

use Carbon\Carbon;
use App\Models\Customer;
use App\Models\Recharge;
use App\Models\InternetPlan;
use Illuminate\Http\Request;

class RechargeController extends Controller
{
    /**
     * Display a listing of the resource.
     */
    public function index()
    {
        //
    }

    /**
     * Show the form for creating a new resource.
     */
    public function create($customerId)
    {
        $customer = Customer::findOrFail($customerId);
        //$internetplans = InternetPlan::get();
        return view('recharges.create', compact('customer'));
    }

    /**
     * Store a newly created resource in storage.
     */
    public function store(Request $request)
    {
        $customer = Customer::findOrFail($request->customer_id);
        $internetplan = InternetPlan::find($request->internetplan_id);

        $rechargeDate = Carbon::now();
        $expiryDate = $rechargeDate->copy()->addMonths($internetplan->duration);

        $latestRecharge = $customer->latestRecharge;

        if($latestRecharge || $latestRecharge->isExpired()) {

            $grace_period = null;

            if($request->has('provide_grace') && (!$latestRecharge || $latestRecharge->hasGracePeriod())) {
            $grace_period = $expiryDate->copy()->addDays(3);

            // Recharge::create([
            //     'customer_id' => $customer->id,
            //     'plan_id' => $plan->id,
            //     'recharge_date' => $rechargeDate,
            //     'expiry_date' => $expiryDate,
            //     'grace_period_end' => $grace_period,
            // ]);

            }

            $recharge = new Recharge();
            $recharge->customer_id = $customer->id;
            $recharge->internet_plan_id = $internetplan->id;
            $recharge->recharge_date = $rechargeDate;
            $recharge->expiry_date = $expiryDate;
            $recharge->grace_period = $grace_period;
            $recharge->save();

            return redirect()->route('customers.index')->with('success', 'Recharge successful' . ($gracePeriodEnd ? ' with grace period added!' : '!'))

        } else {
            return redirect()->route('customers.show', $customer->id)->with('status', 'Recharge not allowed as the current plan is still active.');
        }
    }

    /**
     * Display the specified resource.
     */
    public function show(string $id)
    {
        //
    }

    /**
     * Show the form for editing the specified resource.
     */
    public function edit(string $id)
    {
        //
    }

    /**
     * Update the specified resource in storage.
     */
    public function update(Request $request, string $id)
    {
        //
    }

    /**
     * Remove the specified resource from storage.
     */
    public function destroy(string $id)
    {
        //
    }
}
