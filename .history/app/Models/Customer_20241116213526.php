...
class Customer extends Model
{
    ...
    public function syncWithRad($username, $password, $customerId)
    {
        ...
        if (!$internetPlan || !$internetPlan->rate_limit) {
            // \Log::warning("No valid rate_limit found for customer: {$this->username}");
            return false; // Return false if no valid InternetPlan or rate_limit is found
        }

        $rate_limit = $internetPlan
    ...
    // public function getBandwidth($customerId, $username)
    // {
    //     $customer = self::find($customerId);
    //     $internetPlan = $customer->internetPlan()->with('bandwidth')->first();
