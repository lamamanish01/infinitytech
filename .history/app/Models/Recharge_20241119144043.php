...
class Recharge extends Model
{
    ...
    public function syncWithRad($username, $password, $expire_date, $rate_limit)
    {
        ...
        if ($radCheck) {
            $radCheck->update(['value' => date('d M Y H:i:s', strtotime($expire_date));
        } else {
            RadCheck::updateOrCreate([
                'username' => $username,
                'attribute' => 'Expiration',
                'op' => ':=',
                'value' => date('d M Y H:i:s', strtotime($expire_date)),
            ]);
        }
        ...
        RadReply::where('attribute', 'Mikrotik-Address-List')->delete();
    }
